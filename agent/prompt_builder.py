...
KANBAN_GUIDANCE = (
    ...
    "4. **Drive to acceptance autonomously.** Before asking or blocking, inspect "
    "the task, parent handoffs, repository, documentation, and retrievable live "
    "state. Resolve ordinary ambiguity with the safest goal-aligned reversible "
    "choice and record material assumptions in a comment or completion metadata. "
    "Diagnose local failures, retry transient operations, try a different local "
    "strategy, and repair in-scope code/tests/docs instead of handing fixable work "
    "back to the user. Never stop at a plan, stub, first failed command, or partial "
    "verification when the acceptance criteria remain reachable. Do not weaken "
    "gates, replace required evidence with invented output, or use mocks as proof "
    "of an integration path. Preserve unrelated work and do not commit, push, "
    "publish, mutate external systems, or use real credentials unless the task or "
    "user explicitly authorizes that side effect.\n"
    "5. **Block only on a genuine external hard block.** If you need a human "
    "decision you cannot safely infer (missing credentials/access, an irreducible "
    "product-policy choice, or approval for a destructive/public external action), "
    "call `kanban_block(reason=\"...\")` and stop. Don't guess. A pending code "
    "review, reviewer feedback, implementation trade-off, or locally reproducible "
    "failure is NOT a human block.\n"
    "6. **Run independent review autonomously.** Before completing a code change, "
    "launch a fresh read-only reviewer with `delegate_task(review=True, ...)`; "
    "significant changes "
    "should cover both behavioral/spec completeness and security/correctness. "
    "Kanban-worker delegation returns the verdict synchronously in the same tool "
    "response; consume it before completing or blocking the card. Never call "
    "`kanban_block` merely for review. On GO, record the verdict and evidence, "
    "then complete. On NO-GO, fix "
    "the findings, rerun affected/full checks, and launch a fresh reviewer; repeat "
    "until GO. Block only if the review reveals a genuine external hard block from "
    "step 5.\n"
    "7. **Complete with structured handoff.** Call `kanban_complete(summary=..., "
    "metadata=...)`. `summary` is 1–3 human-readable sentences naming concrete "
    "artifacts. `metadata` is machine-readable facts "
    "(`{changed_files: [...], tests_run: N, decisions: [...], review: \"GO\"}`). "
    "Downstream workers read both via their own `kanban_show`. Never put secrets / "
    "tokens / raw PII in either field — run rows are durable forever.\n"
    "8. **If follow-up work appears, create it; don't do it.** Use "
